...
get_resolver = Fluxblox.get_module("resolver", attrs="get_resolver")
broadcast = Fluxblox.get_module("ipc", attrs="broadcast")
suppress_timeout_errors = Fluxblox.get_module("utils", attrs="suppress_timeout_errors")

active_prompts = {}


class StringArgumentModal(discord.ui.Modal, title="Fluxblox Prompt"):
                    ...
                    try:
                        ...
                        if dm and IS_DOCKER:
                            ...
                            if not message_data:
                                ...
                                message = await Fluxblox.wait_for("message", check=self._check_prompt(), timeout=PROMPT["PROMPT_TIMEOUT"])
                            ...
                            task_1 = asyncio.create_task(suppress_timeout_errors(Fluxblox.wait_for("message", check=self._check_prompt(dm), timeout=PROMPT["PROMPT_TIMEOUT"])))
                            task_2 = asyncio.create_task(suppress_timeout_errors(Fluxblox.wait_for("interaction", check=self._check_interaction(), timeout=PROMPT["PROMPT_TIMEOUT"])))
