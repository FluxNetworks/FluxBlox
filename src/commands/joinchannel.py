...
@Fluxblox.command
class JoinChannelCommand(Fluxblox.Module):
    ...
    async def verified(self, CommandArgs):
        """set the join message of people who are VERIFIED on Fluxblox"""
        ...
        if verified_message:
            response.delete(await response.send("When people join your server and are **VERIFIED** on Fluxblox, this message "
                                                "will be posted:"))
        ...
        await post_event(guild, "configuration", f"{author.mention} ({author.id}) has {change_text} the `joinChannel` option for `verified` members.", BROWN_COLOR)

        raise Message(f"Successfully {change_text} your join message.", type="success")

    @Fluxblox.subcommand()
    async def unverified(self, CommandArgs):
        """set the join message of people who are UNVERIFIED on Fluxblox"""
        ...
        if unverified_message:
            response.delete(await response.send("When people join your server and are **UNVERIFIED** on Fluxblox, this message "
                                                "will be posted:"))
