...
async def register_modules():
    get_files = Fluxblox.get_module("utils", attrs="get_files")

    for directory in MODULE_DIR: # pylint: disable=E1101
        files = get_files(directory)

        for filename in [f.replace(".py", "") for f in files]:
            Fluxblox.get_module(path=directory, dir_name=filename)
...
async def handle_signal(sig):
    ...
    Fluxblox.log(f"Handling signal {sig}")

    loop = asyncio.get_event_loop()

    await Fluxblox.close()
...
if __name__ == "__main__":
    ...
    try:
        ...
        loop.run_until_complete(Fluxblox.close())
        loop.close()
